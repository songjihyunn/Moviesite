		...
		<td align = "right">
			<form action="list.jsp" method="get">
				...
				<select name = "field" style="height: 20px;">
					...
					<option value = 'subject' <%if(field.equals("comment")){%>selected<%}%>>내용</option>
					<option value = 'subject' <%if(field.equals("name")){%>selected<%}%>>글쓴이</option>
				</select>
...
<%
//일반글, 비밀글
String s = "select * from "+code+" where gongji != '1' "+sql_search+ " order by fid desc,thread asc limit "+first+","+num_per_page;
	...
	<tr height=22>
		...
		<%
			...
			<%if( gongji.equals("3") ){//비밀글 %>
				...
				<%} %>
			<%}else{ //일반글 %>
				<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&search=<%=search%>&page_now=<%=page_now%>" ><%=subject %></a>
			<%} %>
